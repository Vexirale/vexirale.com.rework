...
/** Development status bar tinted with the current theme/accent color (album-art
 *  color while music plays). "active" shows an animated indeterminate bar;
 *  "finished" shows a completed bar with a check. */
function StatusBar({
  status,
  accent,
}: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        {finished ? (
          <Check className="h-3 w-3" style={{ color: rgbToRgba(accent, 0.9) }} />
        ) : (
          <span
        ...
        <span
          ...
          style={{ color: rgbToRgba(accent, finished ? 0.6 : 0.9) }}
        >
          {finished ? "Development finished" : "Active development"}
        </span>
      </div>
      <div className="relative h-1 overflow-hidden rounded-full bg-white/10">
        {finished ? (
          <div
            ...
            style={{ background: rgbToRgba(accent, 0.55) }}
          />
        ) : (
          <motion.div
  ...
  const inner = (
    <>
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        {project.status && <StatusBar status={project.status} accent={accent} />}
